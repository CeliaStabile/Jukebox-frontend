  ...
  async function getSuggestions() {
    try {
      const response = await fetch(
        `${backendUrl}/suggestions/${user.partyName}`
      );
    ...
    >
      ...
      >
        <View style={styles.party}>
          ...
          {user.isDj && suggestion.length === 0 && (
            <Text style={styles.title}>Aucune suggestion 😒</Text>
          )}
        </View>
              ...
              return (
                <Swipeable
                  renderLeftActions={(index, song) => (
                    <TouchableOpacity onPress={() => onSwipeableLeftOpen(l)}>
                      <View style={styles.leftSwipeItem}></View>
                    </TouchableOpacity>
                  )}
                  onSwipeableLeftOpen={() => {
                  ...
                  renderRightActions={(index, song) => (
                    <TouchableOpacity onPress={() => onSwipeableRightOpen(l)}>
                      <View style={styles.rightSwipeItem}></View>
                    </TouchableOpacity>
                  )}
                ...
                >
                  <ListItem key={i} bottomDivider style={styles.listitem}>
                    ...
                    >
                      <LikeButton
                        isLiked={personalLikes.includes(l.uri)}
                        song={l}
