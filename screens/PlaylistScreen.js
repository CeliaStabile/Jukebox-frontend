    ...
    if (resQueue.length > 0) {
      try {
        const queueResponse = await fetch(
          `${backendUrl}/queue/copyqueueitems/${user.partyName}`,
          {
            method: "PUT",
    ...
    if (resNowPlaying !== "") {
      try {
        const nowPlayingResponse = await fetch(
          `${backendUrl}/queue/copynowplaying/${user.partyName}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
  ...
  const onRefresh = React.useCallback(async () => {
    ...
    try {
      if (user.isDj) {
        ...
        await fetch(`${backendUrl}/queue/nowplaying/${user.partyName}`, {
          method: "DELETE",
        });
        await getAllSongs();
        await updateQueue();
          ...
          <View style={styles.playnow}>
            ...
            >
              ...
              <ListItem.Content style={styles.playnowcontent}>
                <ListItem.Title style={styles.playnowtitle}>
                  {nowPlaying.title}
                </ListItem.Title>
                <ListItem.Subtitle style={styles.playnowsubtitle}>
                  {nowPlaying.artist}
                </ListItem.Subtitle>
              </ListItem.Content>
